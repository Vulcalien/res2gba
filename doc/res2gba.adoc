...
*u8*. Each tile is made up of 8x8 blocks. To match the behavior of GBA
hardware, the scanning order is: tiles, blocks, pixels.

*images*::
Generic images, converted into arrays of type *u8* or *u16*, depending
on the _bpp_ property. Pixels are scanned line by line.

*palettes*::
Images acting as lists of 16-bit colors, converted into arrays of type
*u16*. Pixels are scanned line by line.

*files*::
Generic files, stored directly into arrays of type *u8* without applying
any conversion. These may contain text, raw audio, level files...
...
*bpp* - integer::
Color depth of the tileset. Allowed values are *4* and *8*.
...
=== Images properties
*bpp* - integer::
Color depth of the image. Allowed values are *8* and *16*.
...
== AUTHORS
*res2gba* was written by Vulcalien.
